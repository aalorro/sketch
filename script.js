...
(function(){
  ...
  document.getElementById('surpriseMe').addEventListener('click', ()=>{
    ...
    document.getElementById('artStyle').value = randomChoice(['pencil','ink','marker','pen','pastel','crayon','coloredpencil']);
    document.getElementById('brush').value = randomChoice(['none','line','hatch','crosshatch','charcoal','inkWash']);
  ...
  function applySketchTransform(ctx, w, h, srcImageData){
    ...
    switch(style) {
      ...
      case 'squiggle': renderSquiggle(ctx, w, h, edges, gray, intensity, stroke, rand, originalColors); break;
    ...
    switch (medium) {
      ...
      case 'crayon': {
        ...
        for (let i = 0; i < n; i++) {
          const v = d[i*4];
          if (v >= BG) {
            d[i*4] = 255; d[i*4+1] = 253; d[i*4+2] = 245; // warm paper
          } else {
      ...
      case 'coloredpencil': {
        ...
        for (let i = 0; i < n; i++) {
          const v = d[i*4];
          if (v >= BG) {
            d[i*4] = 255; d[i*4+1] = 254; d[i*4+2] = 250;
          } else {
  ...
  function renderSquiggle(ctx, w, h, edges, gray, intensity, stroke, rand, originalColors) {
    // Crayon Naïve — per naive_spec.md
    ...
    function wobblePath(pts, amp, freq) {
    ...
    for (let by = 0; by < h; by += blockSize) {
      for (let bx = 0; bx < w; bx += blockSize) {
        ...
        // Color jitter per spec: hue ±5°, sat ±8%, lum ±7%
  ...
  const CANVAS_ONLY_STYLES = new Set(['lineart','crosscontour','scribble','squiggle','photorealism','graphiteportrait','oilpainting','watercolor']);
